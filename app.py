...
@st.cache_data(show_spinner=False, ttl=300)  # ✅ MODIF perf: TTL 5 min
def sql_df(query: str, params: dict | None = None) -> pd.DataFrame:
    with get_conn() as c:
        return pd.read_sql_query(query, c, params=params or {})
...
def assert_db_ready():
    if not DB_PATH.exists():
        st.error("DB introuvable: db/dms.sqlite")
        st.stop()

    views = sql_df("SELECT name FROM sqlite_master WHERE type='view' ORDER BY name")
    if "v_moteurs_dispo" not in set(views["name"].tolist()):
        st.error("Vue v_moteurs_dispo absente")
        st.stop()
...
def add_column_if_missing(table: str, col_def: str):
    ...
    info = sql_df(f"PRAGMA table_info({table})")
    if "name" in info.columns and col_name not in info["name"].tolist():
...
def get_or_create_breaker(name: str) -> int:
    ...
    df = sql_df("SELECT id FROM breakers WHERE name = :name LIMIT 1", {"name": name})
    if not df.empty:
        return int(df.iloc[0]["id"])

    exec_sql("INSERT INTO breakers(name) VALUES(?)", (name,))
    df2 = sql_df("SELECT id FROM breakers WHERE name = :name ORDER BY id DESC LIMIT 1", {"name": name})
...
def get_breaker_stats_today(breaker_id: int) -> dict:
    ...
    SELECT
      (SELECT COUNT(*) FROM breaker_click_offers
        WHERE breaker_id = :bid AND date(created_at) = date('now')) AS n_click,
      (SELECT COUNT(*) FROM breaker_free_offers
        WHERE breaker_id = :bid AND date(created_at) = date('now')) AS n_free
    """
...
def get_ventes_recents(n_months: int) -> pd.DataFrame:
    q = """
    SELECT
      date(em.DateValidation) AS jour,
      strftime('%Y-%m', em.DateValidation) AS mois,
    ...
    WHERE em.DateValidation >= datetime('now', :delta)
    GROUP BY jour, mois, code_moteur, marque, energie
    """
    return sql_df(q, {"delta": f"-{int(n_months)} months"})

def get_ventes_recents_boites(n_months: int) -> pd.DataFrame:
    q = """
    SELECT
      date(eb.DateValidation) AS jour,
      strftime('%Y-%m', eb.DateValidation) AS mois,
      CAST(eb.N_BV AS TEXT) AS code_boite,
      COUNT(*) AS nb_vendus
    FROM "tbl_EXPEDITIONS_boîtes" eb
    WHERE eb.DateValidation >= datetime('now', :delta)
...
def ensure_stock_views():
    # --- BOITES ---
    exec_sql("""
    CREATE VIEW IF NOT EXISTS v_boites_dispo AS
    SELECT
      b.*,
      CASE
...
def get_besoins_moteurs(top_n: int = 50) -> pd.DataFrame:
    ...
    WITH ventes AS (
      ...
      WHERE em.DateValidation >= datetime('now', '-3 months')
      GROUP BY UPPER(m.CodeMoteur)
    ),
    achats AS (
      SELECT
        UPPER(m.CodeMoteur) AS code_moteur,
        AVG(CASE WHEN r.DateAchat >= datetime('now','-3 months')  THEN m.PrixAchatMoteur END) AS prix_moy_3m,
        AVG(CASE WHEN r.DateAchat >= datetime('now','-6 months')  THEN m.PrixAchatMoteur END) AS prix_moy_6m,
        AVG(CASE WHEN r.DateAchat >= datetime('now','-12 months') THEN m.PrixAchatMoteur END) AS prix_moy_12m
      FROM tbl_MOTEURS m
...
def get_besoins_boites(top_n: int = 50) -> pd.DataFrame:
    ...
    WITH ventes AS (
        SELECT
            ...
            COUNT(*) AS nb_vendus_3m
        FROM "tbl_EXPEDITIONS_boîtes" eb
        WHERE eb.DateValidation >= datetime('now', '-3 months')
...
def get_prix_achat_par_mois(n_months: int) -> pd.DataFrame:
    q = """
    SELECT
      strftime('%Y-%m', r.DateAchat) AS mois,
      AVG(m.PrixAchatMoteur) AS prix_achat_moy
    FROM tbl_MOTEURS m
    JOIN tbl_RECEPTIONS r ON r.[N_réception] = m.NumRéception
    WHERE r.DateAchat >= datetime('now', :delta)
...
def get_prix_achat_par_mois_code(n_months: int, code: str) -> pd.DataFrame:
    ...
    ORDER BY mois;
    """
    return sql_df(q, {"delta": f"-{int(n_months)} months", "code": code.upper()})
...
def get_price_movers(
    ...
    if kind == "achat":
        ...
        WITH base AS (
          ...
          FROM tbl_MOTEURS m
          JOIN tbl_RECEPTIONS r ON r.[N_réception] = m.NumRéception
          WHERE r.DateAchat >= datetime('now', '-{int(lookback_months)} months')
            AND m.PrixAchatMoteur IS NOT NULL
            AND m.PrixAchatMoteur > 0
        ),
        agg AS (
          SELECT
            code_moteur,
            AVG(CASE WHEN dt >= datetime('now', '-{int(window_months)} months') THEN prix END) AS avg_recent,
            AVG(CASE WHEN dt <  datetime('now', '-{int(window_months)} months')
                      AND dt >= datetime('now', '-{int(window_months*2)} months') THEN prix END) AS avg_prev,
            SUM(CASE WHEN dt >= datetime('now', '-{int(window_months)} months') THEN 1 ELSE 0 END) AS n_recent,
            SUM(CASE WHEN dt <  datetime('now', '-{int(window_months)} months')
                      AND dt >= datetime('now', '-{int(window_months*2)} months') THEN 1 ELSE 0 END) AS n_prev
          FROM base
        ...
        SELECT
          ...
          n_recent,
          n_prev,
          ROUND(avg_prev, 2) AS avg_prev,
          ROUND(avg_recent, 2) AS avg_recent,
          ROUND((avg_recent - avg_prev), 2) AS delta,
          CASE WHEN avg_prev IS NULL OR avg_prev = 0 THEN NULL
               ELSE ROUND((avg_recent - avg_prev) / avg_prev * 100.0, 2)
          END AS pct
        FROM agg
        ...
        WITH base AS (
          ...
          FROM tbl_EXPEDITIONS_moteurs em
          JOIN tbl_MOTEURS m ON m.N_moteur = em.N_moteur
          WHERE em.DateValidation >= datetime('now', '-{int(lookback_months)} months')
            AND em.PrixVenteMoteur IS NOT NULL
            AND em.PrixVenteMoteur > 0
        ),
        agg AS (
